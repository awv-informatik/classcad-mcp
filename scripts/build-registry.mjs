...
//
// Output: src/method-registry.json — { "v1.<domain>.<method>": { domain, method, summary, paramJSDoc } }
// Consumed at runtime by the call_api / list_methods / describe_method tools.
...
const outputPath = join(root, 'src', 'method-registry.json')
    ...
    if (ts.isVariableStatement(node)) {
      for (const decl of node.declarationList.declarations) {
        if (ts.isIdentifier(decl.name) && decl.name.text === domain && decl.type) {
          // decl.type is a FunctionType: (facade: IFacade) => <ReturnType>
